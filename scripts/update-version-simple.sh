...
###############################################################################
# LeadGen App Form Plugin - Simple Version Update Script
#
...
update_file "${PROJECT_ROOT}/leadgen-app-form.php" "s/Version: [0-9]+\.[0-9]+\.[0-9]+/Version: ${NEW_VERSION}/g"

# Update constant
update_file "${PROJECT_ROOT}/leadgen-app-form.php" "s/define\\(\"LEADGEN_APP_FORM_VERSION\", \"[0-9]+\.[0-9]+\.[0-9]+\"\\)/define(\"LEADGEN_APP_FORM_VERSION\", \"${NEW_VERSION}\")/g"
...
find "${PROJECT_ROOT}/assets/js" "${PROJECT_ROOT}/blocks" -name "*.js" -print0 | while IFS= read -r -d '' file; do
    if grep -q "@version" "$file"; then
        update_file "$file" "s/\@version [0-9]+\.[0-9]+\.[0-9]+/\@version ${NEW_VERSION}/g"
        print_status "  Updated $(basename "$file")"
    fi
...
find "${PROJECT_ROOT}/blocks" -name "block.json" -print0 | while IFS= read -r -d '' file; do
    if grep -q "\"version\":" "$file"; then
        update_file "$file" "s/\"version\": \"[0-9]+\.[0-9]+\.[0-9]+\"/\"version\": \"${NEW_VERSION}\"/g"
        print_status "  Updated $(basename "$file")"
    fi
